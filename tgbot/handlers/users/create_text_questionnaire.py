...
from tgbot.services.dependences import QUEST_ID_LENGTH, MAX_QUESTIONS_QUANTITY
    ...
    while True:
        try:
            ...
            if questions_quantity <= 0:
                ...
                await message.answer("Опрос может состоять не более чем из 15 вопросов. Введите значение снова:")
                return
            else:
                ...
                await db_commands.add_user_created_qe(creator_id=message.from_user.id, quest_id=quest_id)
                await message.answer("Введите 1-й вопрос:", reply_markup=cancel_fill_qe)
...
def register_create_text_qe(dp: Dispatcher):
    dp.register_message_handler(get_qe_title, content_types=types.ContentType.TEXT, state=CreateTextQe.Title)
